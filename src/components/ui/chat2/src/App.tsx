...
import { Send, Wrench, ChevronLeft, Mic, Image as ImageIcon, Info, ChevronDown, ChevronUp, Cpu } from 'lucide-react';
  ...
  return (
    <div className="w-full h-[100dvh] flex flex-col relative overflow-hidden bg-black text-gray-100 font-sans selection:bg-blue-500/30">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 pt-12 bg-gradient-to-t from-black via-black/95 to-transparent z-20">
          <div className={`bg-white/[0.05] backdrop-blur-3xl border rounded-full p-1.5 pl-4 flex items-center gap-3 shadow-[0_10px_40px_rgba(0,0,0,0.8)] shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] transition-colors duration-300 ${isRecording ? 'border-red-500/30 bg-red-500/5' : isObdMode ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/[0.08]'}`}>
            <button 
              onClick={() => setIsObdMode(!isObdMode)}
              className={`p-1.5 rounded-full transition-colors ${isObdMode ? 'text-emerald-400 bg-emerald-500/20' : 'text-gray-400 hover:text-gray-100 hover:bg-white/10'}`}
              title="Skanuj kod OBD-II"
            >
            ...
            <input 
              ...
              placeholder={isRecording ? "Słucham..." : isObdMode ? "Wprowadź kod OBD-II (np. P0300)..." : "Napisz wiadomość..."} 
