...
export function ShazamScanner({ onScanComplete }: ShazamScannerProps) {
  ...
  const simulateProcessing = () => {
    setIsAnalyzing(true);
    setAnalyzingText(mode === 'audio' ? "Izolowanie ścieżki dźwiękowej..." : "Analiza zdjęcia i kształtów...");
    
    setTimeout(() => {
      setAnalyzingText("Przeszukiwanie globalnej bazy modeli...");
    }, 1500);

    setTimeout(() => {
      setAnalyzingText("Identyfikacja...");
    }, 3000);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center font-sans relative overflow-hidden selection:bg-primary/30">
      ...
      <div className="relative z-10 w-full max-w-md mx-auto flex flex-col items-center justify-center h-full px-6">
        ...
        >
          <h1 className="text-4xl font-bold tracking-tighter text-foreground mb-3">Sonic Shazam</h1>
          <p className="text-muted font-medium px-4 text-sm leading-relaxed">
            Nagraj dźwięk silnika lub dodaj zdjęcie, aby inteligentna baza Sonic rozpoznała, z jakim autem masz do czynienia.
          </p>
        </motion.div>
          ...
          <div className="h-12 mt-12 flex items-center justify-center">
             <AnimatePresence mode="wait">
               {isAnalyzing ? (
                 ...
                 >
                   <span className="text-lg font-semibold text-foreground">Słucham...</span>
                   {isDemoMode && <span className="text-[10px] text-muted">Tryb Demo</span>}
                 </motion.div>
               ) : null}
             </AnimatePresence>
          </div>
